...
class ConnectionController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->status == 'توافق') {
            ...
            // Copy m_applicant data to m_marrieds, excluding created_at and updated_at
            $mMarried = new MMarried();
            $mMarried->fill(collect($connection->mApplicant->toArray())->except(['created_at', 'updated_at'])->toArray());
            $mMarried->save();

            // Copy f_applicant data to f_marrieds, excluding created_at and updated_at
            $fMarried = new FMarried();
            $fMarried->fill(collect($connection->fApplicant->toArray())->except(['created_at', 'updated_at'])->toArray());
            ...
            //Check if the m_applicant and f_applicant exist in Disagreements table
            $disagreement = Disagreement::where('m_applicant_id', $connection->mApplicant->id)
                ->first();
            if ($disagreement) {
                // If they exist, delete the disagreement
                $disagreement->delete();
            }
             $disagreement_female = Disagreement::where('f_applicant_id', $connection->fApplicant->id)
                ->first();
            if ($disagreement_female) {
                // If they exist, delete the disagreement   
                $disagreement_female->delete();
            }
         ...
         if ($request->status == 'عدم توافق') {
            ...
            $mAppplicant = $connection->mApplicant;
            $fApplicant = $connection->fApplicant;

            // Copy their id to Disagreements  Table
            Disagreement::create([
                'm_applicant_id' => $mAppplicant->id,
                'f_applicant_id' => $fApplicant->id,
            ]);
            ...
            // Free up their document status 
            $mAppplicant->document_status = true;
            $mAppplicant->save();
            
            $fApplicant->document_status = true;
            $fApplicant->save();
        ...
        if ($request->status == 'عدم توافق') {
            ...
            $connection->fApplicant->document_status = true;
            $connection->fApplicant->save();
        }
